...
begin
    fig = Figure(size = (600, 280), backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))

    ga = fig[1,1] = GridLayout()

    ax1 = Axis(ga[1,1], aspect = DataAspect(), backgroundcolor = RGBAf(0.0,0.0,0.0,0.0), xlabel = "α", ylabel = "β", title = "Flux")

    heatmap!(α, β, img', colormap = :lajolla)
    contour!(α, β, img', colormap = Reverse(:berlin), levels = 10)

    save("rawfigs/page-and-thorne-flux.png", fig, px_per_point = 2)
